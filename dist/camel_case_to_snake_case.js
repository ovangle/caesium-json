...
exports.camelCaseToSnakeCase = {
    encode: (input) => {
        if (!input.match(_MATCH_CAMEL_CASE))
            throw new codec_1.EncodingException(`${input} is not in camel case. Must match ${_MATCH_CAMEL_CASE}`);
        return input.replace(_REPLACE_CAMEL_CASE, (match, p1, p2) => `${p1}_${p2.toLowerCase()}`);
    },
    decode: (input) => {
        if (!input.match(_MATCH_SNAKE_CASE))
            throw new codec_1.EncodingException(`${input} is not in snake_case. Must match ${_MATCH_SNAKE_CASE}`);
