...
import { list } from '../codec';
import { model } from './model';
import { expectThrowsOnNullOrUndefined, stringReversingCodec } from '../utils.spec';
...
describe('model', () => {
    ...
    it('should throw if encoded object has a property not declared on the model', () => {
        ...
        expect(() => codec.decode(encodedValue))
            .toThrow(new Error('\'missingProperty\' not found on \'MyModel\' codec'));
    });
    it('should throw if a required property value is missing', () => {
        ...
        expect(() => codec.decode({}))
            .toThrow(new Error(`Required property 'stringProperty' of 'MyModel' codec not present on object`));
        const codecMissingProperty = model(MyModel, {
            missingProperty: str
        });
        const value = new MyModel();
        expect(() => codecMissingProperty.encode(value))
            .toThrow(new Error(`Required property 'missingProperty' of 'MyModel' codec not present on model`));
    });
