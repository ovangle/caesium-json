...
export function parameterMapTests() {
    describe('SearchParameterMap', () => {
        it('should be possible to get a parameter definition from the map', () => {
            var paramMap = new SearchParameterMap({a: {encoder: identityConverter}});
            expect(paramMap.getParameter('a')).toEqual({encoder: identityConverter});
        ...
        it('should be possible to get the property accessor from a parameter, or to default to the name', () => {
            var paramMap = new SearchParameterMap({
                a: {encoder: identityConverter, accessor: (model) => model.prop},
                b: {encoder: identityConverter}
            });
        ...
        it('should be possible to check whether an object matches a parameter map', () => {
            var paramMap = new SearchParameterMap({
                a: {encoder: identityConverter, matcher: substringRefiner},
                b: {encoder: identityConverter, accessor: (model) => model.propName},
                c: {encoder: identityConverter}
            });
        ...
        it('should be ordered by refinemnet', () => {
            var paramMap = new SearchParameterMap({
                a: {encoder: identityConverter, refiner: substringRefiner},
                b: {encoder: identityConverter}
            });
